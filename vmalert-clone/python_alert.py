...
def query_prometheus(rule):
    query_params = {
        "query": rule["query"]
    }
    url = f'{rule["datasource"]["url"]}?{urlencode(query_params)}'
    try:
...
def export_to_mysql(rule, value):
    cfg = rule["export"]["mysql"]
    try:
